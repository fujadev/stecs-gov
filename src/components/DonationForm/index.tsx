...
const DonationForm: FC<DonationFormProps> = ({ onAmountChange, setIsSuccessOpen, setIsTransactionDetailsOpen }) => {
	...
	const handleSubmit = (values: typeof initialValues) => {
		...
		if (values.paymentMethod == 'Pay with Bank transfer' && values.frequency == 'One Time') {
	...
	return (
		<Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
			{({ errors, handleChange, handleSubmit, setFieldValue, touched, values }) => {
				const selectedPayment = paymentOptions.find((option) => option.label === values.paymentMethod);
				...
				return (
					<form onSubmit={handleSubmit} className="p-[24px] rounded-[12px] w-full  bg-white shadow-two">
						{/* Email Input */}
						<div className="mb-[16px]">
							<label htmlFor="emailInput" className="block text-base font-medium ">
								Email
							</label>
							<input
								id="emailInput"
						...
						{/* Amount Input */}
						<div className="mb-[16px]">
							<label htmlFor="amountInput" className="block text-base font-medium ">
								Amount
							</label>
							<input
								id="amountInput"
							...
							/>
							{touched.amount && errors.amount && <small className="text-red-500">{errors.amount}</small>}
						</div>

						{/* Donation Frequency */}
						<div className="mb-[16px]">
							<label className="block mt-[16px] text-base font-medium ">How often do you want to donate?</label>
							<div className="mt-[4px]">
								<CustomSelect
									name="frequency"
									options={['One Time', 'Weekly']}
									value={values.frequency}
						...
						{/* Weekly Donation */}
						{(values.frequency === 'Weekly' || values.frequency === 'Monthly') && (
							<div>
								<div className="mb-[16px]">
									<label className="block mt-[16px] text-base font-medium ">Select a day during the week</label>
									<div className="mt-[4px]">
										<CustomSelect
											name="weekly"
								...
								<label className="text-base font-medium text-[#07222C] " htmlFor="">
									Payment Choice
								</label>
								...
								<div className="w-full mt-[16px]">
									<p className="text-base font-medium text-[#07222C]">Select when to stop auto-donate</p>
						...
						{values.frequency === 'One Time' && (
							<div>
									...
									{isDropdownOpen && (
										...
										>
											{paymentOptions.map((option) => (
												<label key={option.label} className="flex items-center justify-between px-[8px] py-[12px] cursor-pointer">
								...
								{touched.paymentMethod && errors.paymentMethod && <small className="text-red-500">{errors.paymentMethod}</small>}
							</div>
						)}

						{/* Submit Button */}
						{/* TODO: use proper conditions with id */}
						{values.paymentMethod == 'Pay with Bank transfer' && values.frequency == 'One Time' ? (
							<div className="mt-[28px]">
								...
								>
									<span className="text-[#fff] text-[14px] font-medium flex items-center gap-x-2 ">Make Payment</span>
								</AppButton>
							</div>
						) : (
							<div className="mt-[28px]">
								<PaystackButton {...paystackConfig} className="w-full bg-[#233C8B] text-white py-[14px] px-[16px] rounded-[10px] text-[14px] font-medium">
									Make Payment
								</PaystackButton>
							</div>
						)}
